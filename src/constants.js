...
async function populateTransaction(contract, fragment, args) {
  ...
  if (ro.sequence != null) {
    tx.sequence = new BigNumber(ro.sequence).toNumber()
  }
  if (ro.gasLimit != null) {
    tx.gasLimit = new BigNumber(ro.gasLimit).toNumber()
  }
  if (ro.gasPrice != null) {
    tx.gasPrice = new BigNumber(ro.gasPrice)
  }
  if (ro.from != null) {
    tx.from = ro.from
  }
  ...
  // Populate "thetaValue" override
  if (ro.thetaValue) {
    const roThetaValue = new BigNumber(ro.thetaValue)
    if (!roThetaValue.isZero() && !fragment.payable) {
      //TODO: throw error - non-payable method cannot override value
    }
    tx.thetaValue = roThetaValue
  }
