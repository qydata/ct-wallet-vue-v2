<template>
  <span class="flex w-full overflow-hidden text-white overflow-ellipsis">
    ...
    <div>
      ...
      <svg class="w-20 h-20 mt-2 ml-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"
...
export default {
  ...
  computed: {
    ...
    hashType() {
      if (this.transaction) return this.to === 'explorer' ? 'tx' : 'tx'
      if (this.wallet) return this.to === 'explorer' ? 'address' : 'address'
